...
# 加载数据# 从CSV文件读取数据
data = pd.read_csv('D:\Program Files\JetBrains\pythonProject\huaweiyuce\dataset\dataset/20220823\AMF1.csv', header=0, encoding='gbk')
...
for r in range(repeats):
    # fit 模型
    lstm_model = fit_lstm(train_scaled, 1, 2, 50)  # 训练数据，batch_size，epoche次数, 神经元个数
    # 预测
...
plt.show()
plt.plot(np.arange(258),raw_values[:258], label='True Prices')  # 使用索引作为横坐标
plt.legend()
plt.xlabel('Time Steps')
plt.ylabel('train value')
plt.title('train Prediction using LSTM')
plt.show()
