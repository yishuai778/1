...
for i in range(len(resampled_data)):
    if np.isnan(resampled_data.iloc[i, 0]):
        # 找到第一个非 NaN 值的索引
        first_valid_index = resampled_data.iloc[:i, 0].last_valid_index()
        # 获取前面所有数据
        data_to_difference = resampled_data.iloc[:i, 0].values
        # 进行差分操作
        differenced_data = difference(data_to_difference)
        # 将差分后的数据缩放
        scaled_data_to_predict = scaler.transform(differenced_data.reshape(-1, 1))
        # 将输入数据重塑为模型期望的形状
        X = scaled_data_to_predict[i-2].reshape(1, 1, 1)
        # 使用 LSTM 模型进行预测
        y_pred = lstm_model.predict(X, batch_size=1)
        # 将预测结果逆缩放并逆差分
        filled_value = inverse_difference(data_to_difference, scaler.inverse_transform(y_pred)[0, 0])
        # 更新数据
