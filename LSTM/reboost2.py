...
# 加载数据# 从CSV文件读取数据
data = pd.read_csv('shampoo-sales.csv', header=1, encoding='latin1')

# 将第一列转换为 datetime 类型，第二列转换为数值
data['datetime'] = pd.to_datetime(data.iloc[:, 0], errors='coerce', format='%m/%d')
...
for r in range(repeats):
    # fit 模型
    lstm_model = fit_lstm(train_scaled, 1, 30, 40)  # 训练数据，batch_size，epoche次数, 神经元个数
    # 预测
