...
folder_path = r"D:\Program Files\JetBrains\pythonProject\huaweiyuce\dataset\dataset"
file_name = "AMF1.csv"
found_file = False
...
for root, dirs, files in os.walk(folder_path):
    if file_name in files:
        ...
        # 从CSV文件读取数据
        data = pd.read_csv(file_path, header=1, encoding='latin1')
        datetime_column = pd.to_datetime(data.iloc[:, 0], errors='coerce')

        # 创建包含'datetime'和'value'两列的DataFrame
        series = pd.DataFrame({'datetime': datetime_column, 'value': data.iloc[:, 4]})
        ...
        # 数据拆分：训练数据、测试数据，前80%是训练集，后20%是测试集
        split_ratio = 0.8
        split_index = int(len(supervised_values) * split_ratio)
        ...
        print(file_path)
if not found_file:
    print(f"File {file_name} not found in the specified directory and its subdirectories.")